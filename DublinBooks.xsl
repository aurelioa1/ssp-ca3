	...
	<xsl:template match="/">
		<html>
			<head>
				...
				// This function either turns on or off the row highlighting for ebook
				// items (depending on the value of bShowVeg)
				function highlightebook(idTable, bShowVeg) {
					// if bShowVeg is true, then we're highlighting ebook
					...
					// "ebook" attribute on it.
					for (i = 0; i < aTRs.length; i++) {
						if (aTRs[i].getAttribute('ebook') && aTRs[i].getAttribute('ebook') == "true") {
							if (bShowVeg) {
			...
			<body>
					...
					<tbody>
						<xsl:for-each select="//section">
							...
							<xsl:for-each select="entry">
								<tr>
									<xsl:attribute name="ebook">
										<xsl:value-of select="boolean(@ebook)"/>
									</xsl:attribute>
									<td align="center">
                ...
                Total: €
                <input type="text" name="txtBillAmt"/><input type="checkbox" name="cbOpts" value="isVeg" id="showVeg"/><label for="showVeg">Highlight ebooks</label></p>
				</form>
			</body>
		</html>
	</xsl:template>
